...
let credentials = {
    url: 'https://satolearn.com',
    email: 'aman@vainonex.com',
    password: 'vaio@123'
};
...
if (existsSync('administration.config')){
    connections = JSON.parse(readFileSync('connections.json', 'utf-8'))
...
export function makeid(length, type = 'all') {
    var result = ''
    var characters = ''
    switch (type) {
        case 'all':
...
export function getPath(relativeUrl) {
...
export async function backSpace(elem, num = 0) {
...
export function formatDate(jsDateObj) {
...
export async function findMatchInList(driver, selector, text, optionSelector = 'option', waitFnc = this.wait) {
...
export async function getElementHack(driver, params) {
    const defaults = {
        mode: 'text'
    }
...
export async function getElementsHack(driver, params) {
    const defaults = {
        mode: 'text'
    }
...
export async function serialize(driver, selector) {
...
export async function serialize2(driver, selector) {
    let data = {}
    let form = await driver.findElement(selector)
...
export function decisionMaker(rules, tests) {
    let obj = {}
...
export function getNextWeekday(dayINeed) {
...
export function waitTillPageLoadComplete(browser) {
...
return waitTillPageLoadComplete 
}